...
class TreeMainWin(QtGui.QMainWindow):
    ...
    def editSpeakPasteText(self):
        """Speak paste text from the clipboard"""
        self.editCopy()
        text = self.clipText()
        if not text:
            return
        
        file_object = codecs.open('/Users/lixin/Temp/tts.txt', 'w', 'utf-8')
        try:
            file_object.write(text)
        finally:
            file_object.close()
        p=subprocess.Popen("node /Users/lixin/Workspace/tts/run.js", shell=True)
    ...
    def setupMenus(self):
        ...
        editSpeakPasteTextAct = QtGui.QAction(_('Spe&ak Paste Text'), self)
        editSpeakPasteTextAct.setStatusTip(_('Speak text from the clipboard'))
        editMenu.addAction(editSpeakPasteTextAct)
        self.actions['EditSpeakPasteText'] = editSpeakPasteTextAct
        self.connect(editSpeakPasteTextAct, QtCore.SIGNAL('triggered()'),
                     self.editSpeakPasteText)
