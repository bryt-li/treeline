...
class TreeDoc(object):
    ...
    #add by Jeff
    def readFile_externTpl(self, fileRef, tplRef):
        ...
        filePath = hasattr(fileRef, 'read') and \
                   ...
                   fileRef
        tplPath = hasattr(tplRef, 'read') and \
                   unicode(tplRef.name, sys.getfilesystemencoding()) or \
                   tplRef
        ...
        try:
            ...
            f.close()
            raise ReadFileError(_('readFile_externTpl:Could not open as treeline file'))
        f.close()


        try:
            f = self.getReadFileObj(tplRef)
            f = self.decryptFile(f)
            tpl_handler = treexmlparse.TreeSaxHandler(self)
            input = xml.sax.InputSource()
            input.setByteStream(f)
            input.setEncoding('utf-8')
            reader = xml.sax.make_parser()
            reader.setContentHandler(tpl_handler)
            reader.setFeature(xml.sax.handler.feature_external_ges, 0)
            reader.parse(input)
        except IOError:
            print 'Error - could not read file', \
                  tplPath.encode(globalref.localTextEncoding)
            raise
        except UnicodeError:
            print 'Error - bad Unicode in file', \
                  tplPath.encode(globalref.localTextEncoding)
        ...
        self.treeFormats = TreeFormats(tpl_handler.formats)
    ...
    def readFile(self, fileRef):
        ...
        try:
            ...
            f.close()
            raise ReadFileError(_('readFile:Could not open as treeline file'))
        f.close()
        self.root = handler.rootItem
        self.fileName = filePath
    ...
    #begin add by Jeff
    def exportRawOutput(self, fileRef, nodeList, addBranches=True):
        """Export raw output text file,add by jeff"""
        ...
        for item in nodeList:
            lines.extend(item.exportRawOutput())
    ...
    def getNodeListByTitle(self,title):
        nlist=[]
        next = self.root
        while next:
            if next.title() == title:
