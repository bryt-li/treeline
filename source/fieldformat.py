...
#add by Jeff
class FileFormat(TextFormat):
    """Holds format info for a field with an external file content"""
    typeName = 'File'
    sortSequence = 8
    htmlOption = False
    hasFileBrowse = True
    ...
    def formatOutput(self, storedText, titleMode, internal=False):
        ...
        if titleMode:
            return TextFormat.formatOutput(self, storedText, titleMode,
                                           internal)
        f = codecs.open(storedText, 'r','utf-8')
        content = f.read();
        f.close()
        return content
...
class ScriptFormat(TextFormat):
    """Holds format info for a field with an external script execute output"""
    typeName = 'Script'
    sortSequence = 8
    htmlOption = False
    hasFileBrowse = True
    ...
    def formatOutput(self, storedText, titleMode, internal=False):
        ...
        cmd = storedText
        pipe = subprocess.Popen(cmd, shell=True, bufsize=20480, stdout=subprocess.PIPE).stdout
        lines = [l.decode('utf-8')
