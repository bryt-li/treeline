...
usage = [_('Usage:'),
         ...
         '   -t, --tplfile=%-5s %s' %
            (_('FILE'),
             _('the template filename')),
         '   -r, --rootnode=%-5s %s' %
            (_('NODE'),
             _('specify the node by name, make that node root when export')),
         '   -n, --no-root       %s' %
...
class CmdLine(object):
        ...
        for opt, arg in opts:
            if opt in ('-h', '--help'):
                ...
                if len(args) > 1:
                    printUsage()
                    sys.exit(2)
            elif opt in ('-t', '--tplfile'):
                self.tplfile = arg
                if len(args) > 1:
                    printUsage()
                    sys.exit(2)
            elif opt in ('-r', '--rootnode'):
                self.rootNodeList = arg.split(",")
                if len(args) >1:
                    printUsage()
                    sys.exit(2)
            elif opt in ('-r', '--rootnodeid'):
                self.rootNodeIdList = arg.split(",")
                if len(args) >1:
                    printUsage()
                    sys.exit(2)
    ...
    def importFile(self, fileName, importType):
        ...
        if importType == 'trl':
            importType = 'File'
        if self.tplfile:
            getattr(self.doc, 'read%s_externTpl' % importType.title())(fileName,self.tplfile)
        else:
    ...
    def exportFile(self, oldFileName, exportType):
        ...
        elif exportType == 'rawoutput':
            fileName = self.newFileName(oldFileName, '.txt')
            nodelist = self.getExportRootNodeList()
            self.doc.exportRawOutput(fileName, nodelist)

        return fileName


    def getExportRootNodeList(self):
        nodelist=[]
        for name in self.rootNodeList:
            nlist = self.doc.getNodeListByTitle(name.decode('utf-8'))
            if len(nlist):
                print ('-----Info:node title:"%s" found %d.' % (name,len(nlist)))
                nodelist.extend(nlist)
            else:
                print ('*****Warn:node title:"%s" CAN NOT be found!' % (name))

        if len(nodelist)==0:
            nodelist.append(self.doc.root)
