...
class TreeItem(object):
    ...
    def nodeFormat(self):
        ...
        if globalref.docRef.treeFormats.has_key(self.formatName):
            return globalref.docRef.treeFormats[self.formatName]
        else:
            #print '***Warn: format "%s" not found!' % (self.formatName)
            return None
    ...
    #add by Jeff to support raw output
    def exportRawOutput(self, level=0, openOnly=False):
        """Write raw output text list of descendants: by Jeff"""
        ...
        for line in text:
            mark_line_index = text.index(line)
        ...
        if mark_str_index >= 0:
            alltext=[]
            alltext.extend(text[:mark_line_index])
            alltext.append(mark_line[:mark_str_index])

            for child in self.childList:
                alltext.extend(child.exportRawOutput(level + 1, openOnly))

            alltext.append(mark_line[mark_str_index+len(mark):])
            alltext.extend(text[mark_line_index+1:])
            return alltext

        else:
            for child in self.childList:
                text.extend(child.exportRawOutput(level + 1, openOnly))
            return text
